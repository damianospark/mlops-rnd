...
# Define an entity for the cust. Entities can be thought of as primary keys used to
...
cust_retention_source = BigQuerySource(
    # The BigQuery table where features can be found
    table_ref="test_jacklyn_us.TB_AMT_RETENTION_RESULT_TEST",
    #table_ref="test_jacklyn.TB_AMT_RETENTION_RESULT_202107",    
    # The event timestamp is used for point-in-time joins and for ensuring only
    # features within the TTL are returned
    event_timestamp_column="data_crtn_dt",
...
cust_retention_fv = FeatureView(
    # The unique name of this feature view. Two feature views in a single
    # project cannot have the same name
    name="retention_bain_fv",
    ...
    # for both materialization of features into a store, and are used as references
        features=[
        Feature(name="predict_prob", dtype=ValueType.FLOAT),
        Feature(name="predict_prob_group", dtype=ValueType.FLOAT),
    ],
    ...
    # features
    batch_source=cust_retention_source,
    # Tags are user defined key/value pairs that are attached to each
    # feature view
    tags={"team": "retention_prob"},
)
